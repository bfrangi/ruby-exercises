...
def delete_seat_from_row(chart, row_index, seat_index)
  ...
  chart.dig(row_index).delete_at(seat_index)
  chart
end
...
def count_empty_seats(chart)
  ...
  chart.flatten.filter_map(&:!).length
end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.select { |hash| hash[:is_my_favorite?] }.first
end
